...
var keyD = false, keyA = false, keyS = false, keyW = false; var key_plus = false; var key_less = false;
...
function computeFrame(time) {
    ...
    const esfera = sceneElements.sceneGraph.getObjectByName("esfera");
    const cilindro = sceneElements.sceneGraph.getObjectByName("cilindro");
    ...
    if (key_plus){
        ini_size += 0.05;
        esfera.scale.set(ini_size,ini_size,ini_size)
        key_plus = false;
    }

    if (key_less && ini_size > 0){
        ini_size -= 0.05;
        esfera.scale.set(ini_size,ini_size,ini_size)
        key_less = false;
    }
    ...
    if (right && cilindro.position.x < 2.5) {
        cilindro.translateX(dispX);
    }
    if (up && cilindro.position.z > -2.5) {
        cilindro.translateZ(-dispZ);
    }
    if (left && cilindro.position.x > -2.5) {
        cilindro.translateX(-dispX);
    }
    if (down && cilindro.position.z < 2.5) {
        cilindro.translateZ(dispZ);
    }
